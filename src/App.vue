<template>
  <div class="bg-gray-900 text-white h-dvh max-h-dvh flex flex-col">
    ...
    <div class="flex flex-1 flex-col lg:flex-row h-full min-h-0 overflow-hidden">
      <main class="relative flex-1 p-2 overflow-hidden">
        <div class="absolute top-2 left-2 lg:hidden z-50 flex items-center gap-2">
          ...
          <button v-show="!overlayActive" class="p-2" @click="store.playing = !store.playing">
      ...
      <div class="hidden lg:flex flex-col min-h-0">
        <CharacterSidebar @select="onSelectCharacter" class="lg:w-80" />
      </div>
    ...
    <div v-if="showMobileControls" class="fixed inset-0 z-20 bg-gray-900 lg:hidden flex flex-col">
      <button class="absolute top-3 right-4" @click="showMobileControls = false">✕</button>
